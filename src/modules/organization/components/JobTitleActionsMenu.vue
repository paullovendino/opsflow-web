...
import type { JobTitle } from '@/types/organization'
...
const emit = defineEmits<{
  edit: [jobTitle: JobTitle]
  activate: [jobTitle: JobTitle]
  deactivate: [jobTitle: JobTitle]
  remove: [jobTitle: JobTitle]
}>()
...
function onEdit(): void {
  closeMenu()
  emit('edit', props.jobTitle)
}

function onActivate(): void {
  closeMenu()
  emit('activate', props.jobTitle)
}

function onDeactivate(): void {
  closeMenu()
  emit('deactivate', props.jobTitle)
}

function onRemove(): void {
  closeMenu()
  emit('remove', props.jobTitle)
}
</script>
  ...
  <AppDropdownMenu v-model:open="open" align="end" menu-class="w-44">
    <template #trigger>
      <button
        ...
        :aria-expanded="open"
        :aria-haspopup="true"
        :aria-label="`Actions for ${jobTitle.name}`"
        @click.stop="toggle"
      >
        Actions
      </button>
